...
void orderPizza(orderList* orders, cDict customers, char customerID[], pizzaType pizza, int orderID)
{
    ...
    if(strcmp(customerID,trav->C.customerID) == 0){ //i.e. it found the customer

        //check if there is space in orderList left
        if(orders->numOrders < MAXORDERS){

            //get pizza hash
            int pos = hashOrder(orderID);

            //check if theres a valid position for the newOrder to be created within orderList
            int cmp; //comparisons
            for(cmp = 0;cmp < orders->numOrders && orders->orderList[pos].orderNumber != EMPTY; pos = (pos + 1) % MAXORDERS){
                cmp++;
            }
            ...
            if(newNode != NULL){
                ...
                newNode->nextPizza = orders->orderList[pos].pList; //link
                orders->orderList[pos].pList = newNode; //link


                orders->orderList[pos].stat = OPEN;
